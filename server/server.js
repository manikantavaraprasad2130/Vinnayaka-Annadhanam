...
app.use(cors({
  origin: (origin, callback) => {
    ...
    const matchesLocalhost = /^http:\/\/(localhost|127\.0\.0\.1):\d+$/i.test(origin);
    const matchesVercel = /^https:\/\/.*\.vercel\.app$/i.test(origin);
    const matchesConfigured = allowedOrigins.includes(origin);

    if (matchesLocalhost || matchesVercel || matchesConfigured) {
